...
export default function AddressInputPanel({
  ...
  const [daoFactoryInput, setDaofactoryInput] = useState<string>('')
  const [error, setError] = useState(false)
  const { chainId } = useActiveWeb3React()
  const theme = useContext(ThemeContext)
  const { address, name } = useENS(daoFactoryInput)
  ...
  const handleInput = useCallback(
    event => {
      ...
      setDaofactoryInput(input)
      setError(!onChange(input))
    }
    ,
    [onChange, daoFactoryInput, useENS, address]
  )
      ...
      <ContainerRow error={error}>
        <InputContainer>
          <AutoColumn gap="md">
            ...
            <Input
              className="factory-address-input"
              type="text"
              autoComplete="off"
              autoCorrect="off"
              autoCapitalize="off"
              spellCheck="false"
              placeholder="Factory Address or ENS name"
              error={error}
